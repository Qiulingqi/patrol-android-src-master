 ...
 *
 * The simple way to use this class is to call {@link ImageLoader#get(String, ImageListener)} and to pass in the default image listener provided by {@link ImageLoader#getImageListener(ImageView, int, int)}. Note that all function calls to
...
public class ImageLoader {
	...
	public ImageContainer get( final String requestUrl, ImageListener imageListener, int maxWidth, int maxHeight, Config config ) {
		// only fulfill requests that were initiated from the main thread.
		throwIfNotOnMainThread();
		...
		// The request is not already in flight. Send the new request to the network and
		// track it.
		Request<Bitmap> newRequest = new ImageRequest( requestUrl, new Response.Listener<Bitmap>() {
			...
			public void onErrorResponse( VolleyError paramAnonymousVolleyError ) {
				ImageLoader.this.onGetImageError( cacheKey, paramAnonymousVolleyError );
			}
		} );
	...
	private static String getCacheKey( String url, int maxWidth, int maxHeight ) {
		return new StringBuilder( url.length() + 12 ).append( "#W" ).append( maxWidth ).append( "#H" ).append( maxHeight ).append( "#S" ).append( url ).toString();
	}
}
